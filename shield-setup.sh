...
if [[ $offline_flg -eq 1 ]];then
    ...
    sudo ./install-shield-from-container.sh --version "Rel-${S_APP_VERSION}" --registry $REGISTRY_OVA | tee -a $LOGFILE
...
if [ ! -f ~/.kube/config ] || [ $(cat ~/.kube/config | wc -l) -le 1 ]; then
    ...
    if [[ $offline_flg -eq 0 ]]; then
        curl -s -O https://raw.githubusercontent.com/EricomSoftwareLtd/Shield/${BRANCH}/Kube/scripts/install-rancher-cli.sh
        chmod +x install-rancher-cli.sh
    fi
    ...
    if [[ $offline_flg -eq 0 ]]; then
        curl -s -O https://raw.githubusercontent.com/EricomSoftwareLtd/Shield/${BRANCH}/Kube/scripts/create-cluster.sh
