...
## [VER=20251202a] - 2025/12/02

### 追加

#### shield-recovery-namespace-project.sh
* プロジェクトへのNameスペースの参加処理が失敗した際の復旧スクリプトを追加。

--------------------------------------------------------------
## [VER=20251118a] - 2025/11/18

### 変更

#### kka_monitoring_log.sh
* 取得するログとコマンド結果を追加。
　auth.logを取得。「sudo ls -lha /var/crash/」結果を取得。

--------------------------------------------------------------
## [VER=20250815a] - 2025/08/15

### 変更

#### shield-setup.sh
* WebGL有効化リストが再起動時にリセットされる事象に対するHotFix適用処理を追加。
* 冗長構成の場合にHPA数を調整する処理を追加。

--------------------------------------------------------------
## [VER=20250815a] - 2025/8/15

### 追加

#### kka_monitoring_log.sh
* 定期監視ログ出力とサポートログ取得用のスクリプトを追加。

--------------------------------------------------------------
## [VER=20250604a] - 2025/06/04

### 変更

#### shield-setup.sh
* Sasa v2からSasa v4変更対応。
...
#### rancher_restart.sh
* 23.13-6295.51にて新規追加。証明書更新のためにcronにて日次リスタート。

### 変更

#### shield-start.sh
* 23.13-6295.51にて、rancher_restart.shを追加。cron実装
...
#### shield-prepare-servers.sh
* ユーザパスワードに「$」が含まれて「いない」場合にエラーとなる問題を修正

--------------------------------------------------------------

## [VER=20240213a] - 2024/02/13

### 変更

#### shield-prepare-servers.sh
* ユーザパスワードに「$」が含まれていた場合にエラーとなる問題を修正

--------------------------------------------------------------

## [VER=20240117a] - 2024/01/17

### 変更

#### shield-stop.sh
* -fオプション利用時の、ステータス事前チェックを追加

--------------------------------------------------------------

## [VER=20240115a] - 2024/01/15

### 変更

#### shield-status.sh
* inactiveステータスに対応するためgrepのオプションを変更

--------------------------------------------------------------

## [VER=20231002a] - 2023/10/02
...
## [VER=20230706a] - 2023/07/06

### 変更
#### shield-prepare-servers.sh
* version指定において、Rel- の文字列が付いている場合に削除する処理を追加

#### shield-setup.sh
* 2305におけるリソース修正でyamlの変更を追加(Update時)

--------------------------------------------------------------

## [VER=20230112a] - 2023/01/20
...
* aptのロックを強制解除
* 低メモリ利用設定のスイッチを実装変更
* 外部ログ出力に関わるイメージ差し替えを実装

--------------------------------------------------------------

## [VER=20221114a] - 2022/11/14

### 変更
#### shield-update.sh
* kka_monitoring_log.shとkka_cache_del.shを移動除外対象に追加。
